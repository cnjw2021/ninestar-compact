...
export default function SolarTermsDataPage() {
  const [data, setData] = useState<SolarTermsData[]>([]);
  ...
  const handleChange = (id: number, field: keyof SolarTermsData, value: string | number) => {
    setData(prevData =>
      prevData.map(item =>
    ...
    <div className="container mx-auto py-10">
      <Card className="shadow-lg">
        <CardHeader className="bg-gray-50">
          <CardTitle>二十四節気データ管理</CardTitle>
        </CardHeader>
            ...
            <Table striped highlightOnHover withTableBorder withColumnBorders>
              <Table.Thead>
                <Table.Tr>
                  ...
                  <Table.Th style={{ textAlign: 'center' }}>月</Table.Th>
                  <Table.Th style={{ textAlign: 'center' }}>節気名</Table.Th>
                  <Table.Th style={{ textAlign: 'center' }}>日時</Table.Th>
                  {showTimestamps && (
                    <>
                    ...
                    <Table.Td style={{ textAlign: 'center' }}>
                      {editingId === item.id ? (
                        <TextInput
                          ...
                          value={item.month}
                          style={{ maxWidth: '100px', margin: '0 auto' }}
                          onChange={(e) => handleChange(item.id, 'month', parseInt(e.target.value))}
                        />
                      ) : (
                        item.month
                      )}
                    ...
                    <Table.Td style={{ textAlign: 'center' }}>
                      {editingId === item.id ? (
                        <TextInput
                          ...
                          onChange={(e) => handleChange(item.id, 'term_name', e.target.value)}
                        />
                      ) : (
                        item.term_name
                      )}
