...
export function SolarTermsDataTable() {
  const [data, setData] = useState<SolarTermData[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch('/api/admin/solar-terms', {
        headers: {
  ...
  const handleSave = async (id: number) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/admin/solar-terms/${id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleChange = (id: number, field: keyof SolarTermData, value: string | number) => {
    setData(prevData =>
      prevData.map(item =>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-100">
                ...
                <th className="font-bold text-center px-4 py-3 border-b-2 border-gray-300">月</th>
                <th className="font-bold text-center px-4 py-3 border-b-2 border-gray-300">節気名</th>
                <th className="font-bold text-center px-4 py-3 border-b-2 border-gray-300">日時</th>
                  ...
                  <td className="text-center px-4 py-3 border-b border-gray-200">
                    {editingId === item.id ? (
                      <Input
                        ...
                        value={item.month}
                        className="max-w-[100px] mx-auto"
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(item.id, 'month', parseInt(e.target.value))}
                      />
                    ) : (
                      item.month
                    )}
                  ...
                  <td className="text-center px-4 py-3 border-b border-gray-200">
                    {editingId === item.id ? (
                      <Input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(item.id, 'term_name', e.target.value)}
                      />
                    ) : (
                      item.term_name
                    )}
