...
class MainStarAcquiredFortuneMessage(db.Model):
    """後天定位の運気メッセージモデル
    
    九星ごとに、後天定位の吉運と凶運のメッセージを格納するモデル
    """
    __tablename__ = 'main_star_acquired_fortune_message'
    ...
    def __repr__(self):
        return f"<MainStarAcquiredFortuneMessage(id={self.id}, star={self.star_number})>"
