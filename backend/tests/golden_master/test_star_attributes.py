...
def load_golden_master_data(file_path: str) -> dict:
    """
    '정답' JSON 파일을 로드하는 함수
    (예: "test_star_attributes/case_1.json")
    """
    if not file_path:
    ...
    full_path = os.path.join(current_dir + "/test_star_attributes/", file_path)
...
@pytest.mark.parametrize("input_params, golden_master_file", [
    ...
    ({"star_number": 2}, "case_2.json"),
    ({"star_number": 6}, "case_6.json"),
    ({"star_number": 7}, "case_7.json"),
])
def test_star_attributes(input_params, golden_master_file):
    """
    /api/nine-star/star-attributes 엔드포인트가
    ...
    response = requests.get(
        f"{BASE_URL}/api/nine-star/star-attributes",
        params=input_params
    )
