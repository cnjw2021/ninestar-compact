...
def load_golden_master_data(file_path: str) -> dict:
    """
    '정답' JSON 파일을 로드하는 함수
    (예: "test_month_star_readings/case_19670303.json")
    """
    if not file_path:
    ...
    full_path = os.path.join(current_dir + "/test_month_star_readings/", file_path)
    
    with open(full_path, 'r', encoding='utf-8') as f:
        return json.load(f)

@pytest.mark.parametrize("input_params, golden_master_file", [
    ({"star_number": 1}, "case_1.json"),
    ({"star_number": 5}, "case_5.json"),
    ({"star_number": 9}, "case_9.json"),
])
def test_month_star_readings(input_params, golden_master_file):
    """
    /api/nine-star/month-star-readings 엔드포인트가
    ...
    response = requests.get(
        f"{BASE_URL}/api/nine-star/month-star-readings",
        params=input_params
    )
