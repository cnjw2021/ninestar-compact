...
def load_golden_master_data(file_path: str) -> dict:
    """
    '정답' JSON 파일을 로드하는 함수
    (예: "test_year_star/case_2025.json")
    """
    if not file_path:
    ...
    full_path = os.path.join(current_dir + "/test_year_star/", file_path)
    
    with open(full_path, 'r', encoding='utf-8') as f:
        return json.load(f)

@pytest.mark.parametrize("input_params, golden_master_file", [
    ({"year": YEAR}, "case_2025.json"),
])
def test_year_star(input_params, golden_master_file):
    """
    /api/nine-star/year-star 엔드포인트가
    ...
    response = requests.get(
        f"{BASE_URL}/api/nine-star/year-star",
        params=input_params
    )
