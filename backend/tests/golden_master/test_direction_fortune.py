...
def load_golden_master_data(file_path: str) -> dict:
    """
    '정답' JSON 파일을 로드하는 함수
    (예: "test_direction_fortune/case_1967_6_5.json")
    """
    if not file_path:
    ...
    full_path = os.path.join(current_dir + "/test_direction_fortune/", file_path)
    
    with open(full_path, 'r', encoding='utf-8') as f:
        return json.load(f)

@pytest.mark.parametrize("input_params, golden_master_file", [
    ({"main_star": 6, "month_star": 5, "year": YEAR}, "case_1967_6_5_2025.json"),
    ({"main_star": 2, "month_star": 1, "year": YEAR}, "case_1971_2_1_2025.json"),
    ({"main_star": 1, "month_star": 1, "year": YEAR}, "case_1972_1_1_2025.json"),
    ({"main_star": 7, "month_star": 9, "year": YEAR}, "case_1984_7_9_2025.json"),
])
def test_direction_fortune(input_params, golden_master_file):
    """
    /api/nine-star/direction-fortune 엔드포인트가
